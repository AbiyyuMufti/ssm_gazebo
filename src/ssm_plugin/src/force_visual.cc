...
void ForceVisualPlugin::Load(rendering::VisualPtr _visual, sdf::ElementPtr _sdf)
{
    ...
    this->visual = _visual;
    this->sdf = _sdf;
    auto typeParam = this->sdf->GetAttribute("name");
    std::string name;
    typeParam->Get<std::string>(name);
    gzdbg << "Loading ForceVisual plugin for " << name << std::endl;
...
void ForceVisualPlugin::OnUpdate(ConstForcePtr& force_msg)
{
    ...
    auto force_vector_msg = force_msg->vector();
