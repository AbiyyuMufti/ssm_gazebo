...
#ifndef GAZEBO_MSGS_QUATERNIONCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_QUATERNIONCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Quaternion> gazebo_msgs_QuaternionCustomPtr;
         ...
         * */
        class gazebo_msgs_QuaternionSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_QuaternionSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_QuaternionSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_QuaternionCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_QuaternionSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_QuaternionCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_QuaternionCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_QuaternionCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_QuaternionCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_QuaternionSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_QuaternionSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_QuaternionSubscriber>> m_subscriberPtrMap;
