...
#ifndef GAZEBO_MSGS_POSECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_POSECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Pose> gazebo_msgs_PoseCustomPtr;
         ...
         * */
        class gazebo_msgs_PoseSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_PoseSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_PoseSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_PoseCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_PoseSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_PoseCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_PoseCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_PoseCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_PoseCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_PoseSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_PoseSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_PoseSubscriber>> m_subscriberPtrMap;
