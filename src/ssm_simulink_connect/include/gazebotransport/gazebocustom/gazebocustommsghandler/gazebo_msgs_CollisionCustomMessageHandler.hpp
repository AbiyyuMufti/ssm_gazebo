...
#ifndef GAZEBO_MSGS_COLLISIONCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_COLLISIONCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Collision> gazebo_msgs_CollisionCustomPtr;
         ...
         * */
        class gazebo_msgs_CollisionSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_CollisionSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_CollisionSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_CollisionCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_CollisionSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_CollisionCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_CollisionCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_CollisionCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_CollisionCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_CollisionSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_CollisionSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_CollisionSubscriber>> m_subscriberPtrMap;
