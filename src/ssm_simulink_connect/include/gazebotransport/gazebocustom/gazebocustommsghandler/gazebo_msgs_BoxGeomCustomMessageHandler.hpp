...
#ifndef GAZEBO_MSGS_BOXGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_BOXGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::BoxGeom> gazebo_msgs_BoxGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_BoxGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_BoxGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_BoxGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_BoxGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_BoxGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_BoxGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_BoxGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_BoxGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_BoxGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_BoxGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_BoxGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_BoxGeomSubscriber>> m_subscriberPtrMap;
