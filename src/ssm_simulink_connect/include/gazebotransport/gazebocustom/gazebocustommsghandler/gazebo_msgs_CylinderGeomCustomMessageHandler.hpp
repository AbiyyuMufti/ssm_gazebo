...
#ifndef GAZEBO_MSGS_CYLINDERGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_CYLINDERGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::CylinderGeom> gazebo_msgs_CylinderGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_CylinderGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_CylinderGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_CylinderGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_CylinderGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_CylinderGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_CylinderGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_CylinderGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_CylinderGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_CylinderGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_CylinderGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_CylinderGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_CylinderGeomSubscriber>> m_subscriberPtrMap;
