...
#ifndef GAZEBO_MSGS_VECTOR2DCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_VECTOR2DCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Vector2d> gazebo_msgs_Vector2dCustomPtr;
         ...
         * */
        class gazebo_msgs_Vector2dSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_Vector2dSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_Vector2dSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_Vector2dCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_Vector2dSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_Vector2dCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_Vector2dCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_Vector2dCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_Vector2dCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_Vector2dSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_Vector2dSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_Vector2dSubscriber>> m_subscriberPtrMap;
