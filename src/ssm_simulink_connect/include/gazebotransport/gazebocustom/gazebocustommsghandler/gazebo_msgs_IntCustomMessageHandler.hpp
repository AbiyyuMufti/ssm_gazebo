...
#ifndef GAZEBO_MSGS_INTCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_INTCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Int> gazebo_msgs_IntCustomPtr;
         ...
         * */
        class gazebo_msgs_IntSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_IntSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_IntSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_IntCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_IntSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_IntCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_IntCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_IntCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_IntCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_IntSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_IntSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_IntSubscriber>> m_subscriberPtrMap;
