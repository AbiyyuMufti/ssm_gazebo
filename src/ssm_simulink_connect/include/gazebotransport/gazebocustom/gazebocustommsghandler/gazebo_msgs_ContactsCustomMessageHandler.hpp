...
#ifndef GAZEBO_MSGS_CONTACTSCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_CONTACTSCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Contacts> gazebo_msgs_ContactsCustomPtr;
         ...
         * */
        class gazebo_msgs_ContactsSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_ContactsSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_ContactsSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_ContactsCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_ContactsSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_ContactsCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_ContactsCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_ContactsCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_ContactsCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_ContactsSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_ContactsSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_ContactsSubscriber>> m_subscriberPtrMap;
