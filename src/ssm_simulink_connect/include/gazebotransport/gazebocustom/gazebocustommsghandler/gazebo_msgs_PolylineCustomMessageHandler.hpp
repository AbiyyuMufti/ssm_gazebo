...
#ifndef GAZEBO_MSGS_POLYLINECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_POLYLINECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Polyline> gazebo_msgs_PolylineCustomPtr;
         ...
         * */
        class gazebo_msgs_PolylineSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_PolylineSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_PolylineSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_PolylineCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_PolylineSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_PolylineCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_PolylineCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_PolylineCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_PolylineCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_PolylineSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_PolylineSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_PolylineSubscriber>> m_subscriberPtrMap;
