...
#ifndef GAZEBO_MSGS_IMAGEGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_IMAGEGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::ImageGeom> gazebo_msgs_ImageGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_ImageGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_ImageGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_ImageGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_ImageGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_ImageGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_ImageGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_ImageGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_ImageGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_ImageGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_ImageGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_ImageGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_ImageGeomSubscriber>> m_subscriberPtrMap;
