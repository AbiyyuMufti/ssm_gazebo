...
#ifndef GAZEBO_MSGS_IMUSENSORCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_IMUSENSORCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::IMUSensor> gazebo_msgs_IMUSensorCustomPtr;
         ...
         * */
        class gazebo_msgs_IMUSensorSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_IMUSensorSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_IMUSensorSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_IMUSensorCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_IMUSensorSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_IMUSensorCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_IMUSensorCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_IMUSensorCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_IMUSensorCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_IMUSensorSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_IMUSensorSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_IMUSensorSubscriber>> m_subscriberPtrMap;
