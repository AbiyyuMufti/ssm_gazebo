...
#ifndef GAZEBO_MSGS_TIMECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_TIMECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Time> gazebo_msgs_TimeCustomPtr;
         ...
         * */
        class gazebo_msgs_TimeSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_TimeSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_TimeSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_TimeCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_TimeSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_TimeCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_TimeCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_TimeCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_TimeCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_TimeSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_TimeSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_TimeSubscriber>> m_subscriberPtrMap;
