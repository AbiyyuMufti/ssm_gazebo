...
#ifndef GAZEBO_MSGS_PLANEGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_PLANEGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::PlaneGeom> gazebo_msgs_PlaneGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_PlaneGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_PlaneGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_PlaneGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_PlaneGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_PlaneGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_PlaneGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_PlaneGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_PlaneGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_PlaneGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_PlaneGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_PlaneGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_PlaneGeomSubscriber>> m_subscriberPtrMap;
