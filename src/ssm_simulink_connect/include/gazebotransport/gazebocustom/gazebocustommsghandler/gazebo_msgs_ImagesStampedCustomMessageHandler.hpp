...
#ifndef GAZEBO_MSGS_IMAGESSTAMPEDCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_IMAGESSTAMPEDCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::ImagesStamped> gazebo_msgs_ImagesStampedCustomPtr;
         ...
         * */
        class gazebo_msgs_ImagesStampedSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_ImagesStampedSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_ImagesStampedSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_ImagesStampedCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_ImagesStampedSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_ImagesStampedCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_ImagesStampedCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_ImagesStampedCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_ImagesStampedCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_ImagesStampedSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_ImagesStampedSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_ImagesStampedSubscriber>> m_subscriberPtrMap;
