...
#ifndef GAZEBO_MSGS_CONTACTCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_CONTACTCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Contact> gazebo_msgs_ContactCustomPtr;
         ...
         * */
        class gazebo_msgs_ContactSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_ContactSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_ContactSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_ContactCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_ContactSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_ContactCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_ContactCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_ContactCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_ContactCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_ContactSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_ContactSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_ContactSubscriber>> m_subscriberPtrMap;
