...
#ifndef GAZEBO_MSGS_SURFACECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_SURFACECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Surface> gazebo_msgs_SurfaceCustomPtr;
         ...
         * */
        class gazebo_msgs_SurfaceSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_SurfaceSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_SurfaceSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_SurfaceCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_SurfaceSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_SurfaceCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_SurfaceCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_SurfaceCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_SurfaceCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_SurfaceSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_SurfaceSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_SurfaceSubscriber>> m_subscriberPtrMap;
