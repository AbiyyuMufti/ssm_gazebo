...
#ifndef GAZEBO_MSGS_GEOMETRYCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_GEOMETRYCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Geometry> gazebo_msgs_GeometryCustomPtr;
         ...
         * */
        class gazebo_msgs_GeometrySubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_GeometrySubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_GeometrySubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_GeometryCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_GeometrySubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_GeometryCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_GeometryCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_GeometryCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_GeometryCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_GeometrySubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_GeometrySubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_GeometrySubscriber>> m_subscriberPtrMap;
