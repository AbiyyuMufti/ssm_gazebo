...
#ifndef GAZEBO_MSGS_INERTIALCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_INERTIALCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Inertial> gazebo_msgs_InertialCustomPtr;
         ...
         * */
        class gazebo_msgs_InertialSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_InertialSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_InertialSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_InertialCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_InertialSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_InertialCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_InertialCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_InertialCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_InertialCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_InertialSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_InertialSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_InertialSubscriber>> m_subscriberPtrMap;
