...
#ifndef GAZEBO_MSGS_SPHEREGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_SPHEREGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::SphereGeom> gazebo_msgs_SphereGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_SphereGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_SphereGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_SphereGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_SphereGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_SphereGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_SphereGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_SphereGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_SphereGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_SphereGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_SphereGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_SphereGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_SphereGeomSubscriber>> m_subscriberPtrMap;
