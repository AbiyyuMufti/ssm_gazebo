...
#ifndef GAZEBO_MSGS_COLORCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_COLORCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Color> gazebo_msgs_ColorCustomPtr;
         ...
         * */
        class gazebo_msgs_ColorSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_ColorSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_ColorSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_ColorCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_ColorSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_ColorCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_ColorCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_ColorCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_ColorCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_ColorSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_ColorSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_ColorSubscriber>> m_subscriberPtrMap;
