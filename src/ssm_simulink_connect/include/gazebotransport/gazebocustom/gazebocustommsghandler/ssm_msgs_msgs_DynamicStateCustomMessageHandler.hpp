...
#ifndef SSM_MSGS_MSGS_DYNAMICSTATECUSTOMMESSAGEHANDLER_HPP
#define SSM_MSGS_MSGS_DYNAMICSTATECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const ssm_msgs::msgs::DynamicState> ssm_msgs_msgs_DynamicStateCustomPtr;
         ...
         * */
        class ssm_msgs_msgs_DynamicStateSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit ssm_msgs_msgs_DynamicStateSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~ssm_msgs_msgs_DynamicStateSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::ssm_msgs_msgs_DynamicStateCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &ssm_msgs_msgs_DynamicStateSubscriber::OnUpdate, this);
                }
            }
         ...
         * The ssm_msgs_msgs_DynamicStateCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class ssm_msgs_msgs_DynamicStateCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit ssm_msgs_msgs_DynamicStateCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~ssm_msgs_msgs_DynamicStateCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<ssm_msgs_msgs_DynamicStateSubscriber>, bool> insert(
            ...
            std::shared_ptr<ssm_msgs_msgs_DynamicStateSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<ssm_msgs_msgs_DynamicStateSubscriber>> m_subscriberPtrMap;
