...
#ifndef GAZEBO_MSGS_MATERIALCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_MATERIALCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Material> gazebo_msgs_MaterialCustomPtr;
         ...
         * */
        class gazebo_msgs_MaterialSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_MaterialSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_MaterialSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_MaterialCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_MaterialSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_MaterialCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_MaterialCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_MaterialCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_MaterialCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_MaterialSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_MaterialSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_MaterialSubscriber>> m_subscriberPtrMap;
