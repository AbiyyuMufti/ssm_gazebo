...
#ifndef GAZEBO_MSGS_DOUBLECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_DOUBLECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Double> gazebo_msgs_DoubleCustomPtr;
         ...
         * */
        class gazebo_msgs_DoubleSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_DoubleSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_DoubleSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_DoubleCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_DoubleSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_DoubleCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_DoubleCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_DoubleCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_DoubleCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_DoubleSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_DoubleSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_DoubleSubscriber>> m_subscriberPtrMap;
