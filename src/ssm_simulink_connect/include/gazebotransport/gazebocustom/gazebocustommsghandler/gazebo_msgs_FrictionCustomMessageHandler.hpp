...
#ifndef GAZEBO_MSGS_FRICTIONCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_FRICTIONCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Friction> gazebo_msgs_FrictionCustomPtr;
         ...
         * */
        class gazebo_msgs_FrictionSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_FrictionSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_FrictionSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_FrictionCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_FrictionSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_FrictionCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_FrictionCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_FrictionCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_FrictionCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_FrictionSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_FrictionSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_FrictionSubscriber>> m_subscriberPtrMap;
