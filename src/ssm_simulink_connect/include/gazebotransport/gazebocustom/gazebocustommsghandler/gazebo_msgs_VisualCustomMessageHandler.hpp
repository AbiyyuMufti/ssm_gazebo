...
#ifndef GAZEBO_MSGS_VISUALCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_VISUALCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Visual> gazebo_msgs_VisualCustomPtr;
         ...
         * */
        class gazebo_msgs_VisualSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_VisualSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_VisualSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_VisualCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_VisualSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_VisualCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_VisualCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_VisualCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_VisualCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_VisualSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_VisualSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_VisualSubscriber>> m_subscriberPtrMap;
