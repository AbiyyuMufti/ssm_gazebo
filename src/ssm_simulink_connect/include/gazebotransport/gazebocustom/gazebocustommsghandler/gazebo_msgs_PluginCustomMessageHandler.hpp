...
#ifndef GAZEBO_MSGS_PLUGINCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_PLUGINCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Plugin> gazebo_msgs_PluginCustomPtr;
         ...
         * */
        class gazebo_msgs_PluginSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_PluginSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_PluginSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_PluginCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_PluginSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_PluginCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_PluginCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_PluginCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_PluginCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_PluginSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_PluginSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_PluginSubscriber>> m_subscriberPtrMap;
