...
#ifndef GAZEBO_MSGS_SENSORNOISECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_SENSORNOISECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::SensorNoise> gazebo_msgs_SensorNoiseCustomPtr;
         ...
         * */
        class gazebo_msgs_SensorNoiseSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_SensorNoiseSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_SensorNoiseSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_SensorNoiseCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_SensorNoiseSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_SensorNoiseCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_SensorNoiseCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_SensorNoiseCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_SensorNoiseCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_SensorNoiseSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_SensorNoiseSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_SensorNoiseSubscriber>> m_subscriberPtrMap;
