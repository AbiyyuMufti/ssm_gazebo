...
#ifndef SSM_MSGS_MSGS_SPHERICALVECTORCUSTOMMESSAGEHANDLER_HPP
#define SSM_MSGS_MSGS_SPHERICALVECTORCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const ssm_msgs::msgs::SphericalVector> ssm_msgs_msgs_SphericalVectorCustomPtr;
         ...
         * */
        class ssm_msgs_msgs_SphericalVectorSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit ssm_msgs_msgs_SphericalVectorSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~ssm_msgs_msgs_SphericalVectorSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::ssm_msgs_msgs_SphericalVectorCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &ssm_msgs_msgs_SphericalVectorSubscriber::OnUpdate, this);
                }
            }
         ...
         * The ssm_msgs_msgs_SphericalVectorCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class ssm_msgs_msgs_SphericalVectorCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit ssm_msgs_msgs_SphericalVectorCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~ssm_msgs_msgs_SphericalVectorCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<ssm_msgs_msgs_SphericalVectorSubscriber>, bool> insert(
            ...
            std::shared_ptr<ssm_msgs_msgs_SphericalVectorSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<ssm_msgs_msgs_SphericalVectorSubscriber>> m_subscriberPtrMap;
