...
#ifndef GAZEBO_MSGS_JOINTWRENCHCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_JOINTWRENCHCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::JointWrench> gazebo_msgs_JointWrenchCustomPtr;
         ...
         * */
        class gazebo_msgs_JointWrenchSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_JointWrenchSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_JointWrenchSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_JointWrenchCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_JointWrenchSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_JointWrenchCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_JointWrenchCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_JointWrenchCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_JointWrenchCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_JointWrenchSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_JointWrenchSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_JointWrenchSubscriber>> m_subscriberPtrMap;
