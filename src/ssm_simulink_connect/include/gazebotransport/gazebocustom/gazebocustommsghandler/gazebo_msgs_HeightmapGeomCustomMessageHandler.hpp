...
#ifndef GAZEBO_MSGS_HEIGHTMAPGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_HEIGHTMAPGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::HeightmapGeom> gazebo_msgs_HeightmapGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_HeightmapGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_HeightmapGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_HeightmapGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_HeightmapGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_HeightmapGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_HeightmapGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_HeightmapGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_HeightmapGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_HeightmapGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_HeightmapGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_HeightmapGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_HeightmapGeomSubscriber>> m_subscriberPtrMap;
