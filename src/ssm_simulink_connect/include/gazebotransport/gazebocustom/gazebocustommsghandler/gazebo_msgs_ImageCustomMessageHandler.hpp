...
#ifndef GAZEBO_MSGS_IMAGECUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_IMAGECUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::Image> gazebo_msgs_ImageCustomPtr;
         ...
         * */
        class gazebo_msgs_ImageSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_ImageSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_ImageSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_ImageCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_ImageSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_ImageCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_ImageCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_ImageCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_ImageCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_ImageSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_ImageSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_ImageSubscriber>> m_subscriberPtrMap;
