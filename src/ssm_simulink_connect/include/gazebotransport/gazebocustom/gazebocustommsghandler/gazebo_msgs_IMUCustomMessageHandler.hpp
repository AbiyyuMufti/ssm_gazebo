...
#ifndef GAZEBO_MSGS_IMUCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_IMUCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::IMU> gazebo_msgs_IMUCustomPtr;
         ...
         * */
        class gazebo_msgs_IMUSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_IMUSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_IMUSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_IMUCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_IMUSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_IMUCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_IMUCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_IMUCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_IMUCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_IMUSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_IMUSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_IMUSubscriber>> m_subscriberPtrMap;
