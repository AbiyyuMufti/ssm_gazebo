...
#ifndef GAZEBO_MSGS_MESHGEOMCUSTOMMESSAGEHANDLER_HPP
#define GAZEBO_MSGS_MESHGEOMCUSTOMMESSAGEHANDLER_HPP
...
namespace robotics {
    namespace gazebotransport {
        typedef const boost::shared_ptr<const gazebo::msgs::MeshGeom> gazebo_msgs_MeshGeomCustomPtr;
         ...
         * */
        class gazebo_msgs_MeshGeomSubscriber {
        public:
            /// Constructor
            /*
             ...
             */
            explicit gazebo_msgs_MeshGeomSubscriber(std::string const& topicName)
            ...
            /// Destructor
            virtual ~gazebo_msgs_MeshGeomSubscriber() {
            }
            
            // subscriber callback which subscribe custom message from specific topic
            // and stores in the custom message container
            void OnUpdate(robotics::gazebotransport::gazebo_msgs_MeshGeomCustomPtr& _msg) {
                {
            ...
            void InitImpl(gazebo::transport::NodePtr node) {
                if (!m_subscriberPtr) {
                    m_subscriberPtr = node->Subscribe(m_topicName, &gazebo_msgs_MeshGeomSubscriber::OnUpdate, this);
                }
            }
         ...
         * The gazebo_msgs_MeshGeomCustomMessageHandler initialize gazebo publisher, subscriber.
         * Further, it publish and subscribe custom message on/from gazebo.
         * It can support multiple topics of same message type.
         * */
        class gazebo_msgs_MeshGeomCustomMessageHandler : public CustomMsgHandler {
        public:
            /// Constructor
            /*
			@param node                     Gazebo transport node pointer
			@param customContainer          Custom message storage instance
             */
            explicit gazebo_msgs_MeshGeomCustomMessageHandler(gazebo::transport::NodePtr node);
            
            /// Destructor
            ~gazebo_msgs_MeshGeomCustomMessageHandler();
            ...
            std::pair<std::shared_ptr<gazebo_msgs_MeshGeomSubscriber>, bool> insert(
            ...
            std::shared_ptr<gazebo_msgs_MeshGeomSubscriber> find(std::string const& topic);
        ...
        private:
            ...
            std::map<std::string, std::shared_ptr<gazebo_msgs_MeshGeomSubscriber>> m_subscriberPtrMap;
