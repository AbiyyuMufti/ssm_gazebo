...
gazebo_msgs_ImageGeomCustomMessageHandler::gazebo_msgs_ImageGeomCustomMessageHandler(gazebo::transport::NodePtr node)
    : m_node(node)
    , m_mutex()
{
}
...
/// Destructor
gazebo_msgs_ImageGeomCustomMessageHandler::~gazebo_msgs_ImageGeomCustomMessageHandler() {
}

void gazebo_msgs_ImageGeomCustomMessageHandler::setCustomPub(gazebo::transport::PublisherPtr customPubPtr,
                                          std::string const& topicName) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_customPublisherMap.insert(std::make_pair(topicName, customPubPtr));
}

gazebo::transport::PublisherPtr gazebo_msgs_ImageGeomCustomMessageHandler::getCustomPub(std::string const& topicName) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_ImageGeomCustomMessageHandler::initPublisher(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_customPublisherMap.end()) {
        ...
        this->setCustomPub(this->m_node->Advertise<gazebo::msgs::ImageGeom>(topicName),
                           topicName);
        replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
    }
...
std::pair<std::shared_ptr<gazebo_msgs_ImageGeomSubscriber>, bool> gazebo_msgs_ImageGeomCustomMessageHandler::insert(
    std::string const& topic_name,
    std::shared_ptr<gazebo_msgs_ImageGeomSubscriber> subsciberPointer) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto insertResult =
...
std::shared_ptr<gazebo_msgs_ImageGeomSubscriber> gazebo_msgs_ImageGeomCustomMessageHandler::find(std::string const& topic) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_ImageGeomCustomMessageHandler::initSubscriber(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_subscriberPtrMap.end()) {
        ...
        auto insertResult =
            this->insert(topicName, std::make_shared<gazebo_msgs_ImageGeomSubscriber>(topicName));
...
std::string gazebo_msgs_ImageGeomCustomMessageHandler::publishCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (pubPointer) {
        gazebo::msgs::ImageGeom gazebo_msgs_ImageGeom_message;
        if (gazebo_msgs_ImageGeom_message.ParseFromString(dataString)) {
            pubPointer->Publish(gazebo_msgs_ImageGeom_message);
            // assign no error
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
        } else {
...
std::pair<bool,std::string>  gazebo_msgs_ImageGeomCustomMessageHandler::subscribeCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
