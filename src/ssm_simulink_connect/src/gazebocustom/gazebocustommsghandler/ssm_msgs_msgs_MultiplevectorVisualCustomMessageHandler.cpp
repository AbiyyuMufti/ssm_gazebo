...
ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler(gazebo::transport::NodePtr node)
    : m_node(node)
    , m_mutex()
{
}
...
/// Destructor
ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::~ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler() {
}

void ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::setCustomPub(gazebo::transport::PublisherPtr customPubPtr,
                                          std::string const& topicName) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_customPublisherMap.insert(std::make_pair(topicName, customPubPtr));
}

gazebo::transport::PublisherPtr ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::getCustomPub(std::string const& topicName) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
std::string ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::initPublisher(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_customPublisherMap.end()) {
        ...
        this->setCustomPub(this->m_node->Advertise<ssm_msgs::msgs::MultiplevectorVisual>(topicName),
                           topicName);
        replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
    }
...
std::pair<std::shared_ptr<ssm_msgs_msgs_MultiplevectorVisualSubscriber>, bool> ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::insert(
    std::string const& topic_name,
    std::shared_ptr<ssm_msgs_msgs_MultiplevectorVisualSubscriber> subsciberPointer) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto insertResult =
...
std::shared_ptr<ssm_msgs_msgs_MultiplevectorVisualSubscriber> ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::find(std::string const& topic) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::initSubscriber(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_subscriberPtrMap.end()) {
        ...
        auto insertResult =
            this->insert(topicName, std::make_shared<ssm_msgs_msgs_MultiplevectorVisualSubscriber>(topicName));
...
std::string ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::publishCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (pubPointer) {
        ssm_msgs::msgs::MultiplevectorVisual ssm_msgs_msgs_MultiplevectorVisual_message;
        if (ssm_msgs_msgs_MultiplevectorVisual_message.ParseFromString(dataString)) {
            pubPointer->Publish(ssm_msgs_msgs_MultiplevectorVisual_message);
            // assign no error
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
        } else {
...
std::pair<bool,std::string>  ssm_msgs_msgs_MultiplevectorVisualCustomMessageHandler::subscribeCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
