...
gazebo_msgs_MeshGeomCustomMessageHandler::gazebo_msgs_MeshGeomCustomMessageHandler(gazebo::transport::NodePtr node)
    : m_node(node)
    , m_mutex()
{
}
...
/// Destructor
gazebo_msgs_MeshGeomCustomMessageHandler::~gazebo_msgs_MeshGeomCustomMessageHandler() {
}

void gazebo_msgs_MeshGeomCustomMessageHandler::setCustomPub(gazebo::transport::PublisherPtr customPubPtr,
                                          std::string const& topicName) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_customPublisherMap.insert(std::make_pair(topicName, customPubPtr));
}

gazebo::transport::PublisherPtr gazebo_msgs_MeshGeomCustomMessageHandler::getCustomPub(std::string const& topicName) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_MeshGeomCustomMessageHandler::initPublisher(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_customPublisherMap.end()) {
        ...
        this->setCustomPub(this->m_node->Advertise<gazebo::msgs::MeshGeom>(topicName),
                           topicName);
        replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
    }
...
std::pair<std::shared_ptr<gazebo_msgs_MeshGeomSubscriber>, bool> gazebo_msgs_MeshGeomCustomMessageHandler::insert(
    std::string const& topic_name,
    std::shared_ptr<gazebo_msgs_MeshGeomSubscriber> subsciberPointer) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto insertResult =
...
std::shared_ptr<gazebo_msgs_MeshGeomSubscriber> gazebo_msgs_MeshGeomCustomMessageHandler::find(std::string const& topic) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_MeshGeomCustomMessageHandler::initSubscriber(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_subscriberPtrMap.end()) {
        ...
        auto insertResult =
            this->insert(topicName, std::make_shared<gazebo_msgs_MeshGeomSubscriber>(topicName));
...
std::string gazebo_msgs_MeshGeomCustomMessageHandler::publishCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (pubPointer) {
        gazebo::msgs::MeshGeom gazebo_msgs_MeshGeom_message;
        if (gazebo_msgs_MeshGeom_message.ParseFromString(dataString)) {
            pubPointer->Publish(gazebo_msgs_MeshGeom_message);
            // assign no error
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
        } else {
...
std::pair<bool,std::string>  gazebo_msgs_MeshGeomCustomMessageHandler::subscribeCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
