...
ssm_msgs_msgs_SphericalVectorCustomMessageHandler::ssm_msgs_msgs_SphericalVectorCustomMessageHandler(gazebo::transport::NodePtr node)
    : m_node(node)
    , m_mutex()
{
}
...
/// Destructor
ssm_msgs_msgs_SphericalVectorCustomMessageHandler::~ssm_msgs_msgs_SphericalVectorCustomMessageHandler() {
}

void ssm_msgs_msgs_SphericalVectorCustomMessageHandler::setCustomPub(gazebo::transport::PublisherPtr customPubPtr,
                                          std::string const& topicName) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_customPublisherMap.insert(std::make_pair(topicName, customPubPtr));
}

gazebo::transport::PublisherPtr ssm_msgs_msgs_SphericalVectorCustomMessageHandler::getCustomPub(std::string const& topicName) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
std::string ssm_msgs_msgs_SphericalVectorCustomMessageHandler::initPublisher(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_customPublisherMap.end()) {
        ...
        this->setCustomPub(this->m_node->Advertise<ssm_msgs::msgs::SphericalVector>(topicName),
                           topicName);
        replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
    }
...
std::pair<std::shared_ptr<ssm_msgs_msgs_SphericalVectorSubscriber>, bool> ssm_msgs_msgs_SphericalVectorCustomMessageHandler::insert(
    std::string const& topic_name,
    std::shared_ptr<ssm_msgs_msgs_SphericalVectorSubscriber> subsciberPointer) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto insertResult =
...
std::shared_ptr<ssm_msgs_msgs_SphericalVectorSubscriber> ssm_msgs_msgs_SphericalVectorCustomMessageHandler::find(std::string const& topic) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string ssm_msgs_msgs_SphericalVectorCustomMessageHandler::initSubscriber(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_subscriberPtrMap.end()) {
        ...
        auto insertResult =
            this->insert(topicName, std::make_shared<ssm_msgs_msgs_SphericalVectorSubscriber>(topicName));
...
std::string ssm_msgs_msgs_SphericalVectorCustomMessageHandler::publishCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (pubPointer) {
        ssm_msgs::msgs::SphericalVector ssm_msgs_msgs_SphericalVector_message;
        if (ssm_msgs_msgs_SphericalVector_message.ParseFromString(dataString)) {
            pubPointer->Publish(ssm_msgs_msgs_SphericalVector_message);
            // assign no error
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
        } else {
...
std::pair<bool,std::string>  ssm_msgs_msgs_SphericalVectorCustomMessageHandler::subscribeCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
