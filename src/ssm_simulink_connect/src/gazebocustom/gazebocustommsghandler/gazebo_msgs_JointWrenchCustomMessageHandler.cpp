...
gazebo_msgs_JointWrenchCustomMessageHandler::gazebo_msgs_JointWrenchCustomMessageHandler(gazebo::transport::NodePtr node)
    : m_node(node)
    , m_mutex()
{
}
...
/// Destructor
gazebo_msgs_JointWrenchCustomMessageHandler::~gazebo_msgs_JointWrenchCustomMessageHandler() {
}

void gazebo_msgs_JointWrenchCustomMessageHandler::setCustomPub(gazebo::transport::PublisherPtr customPubPtr,
                                          std::string const& topicName) {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_customPublisherMap.insert(std::make_pair(topicName, customPubPtr));
}

gazebo::transport::PublisherPtr gazebo_msgs_JointWrenchCustomMessageHandler::getCustomPub(std::string const& topicName) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_JointWrenchCustomMessageHandler::initPublisher(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_customPublisherMap.end()) {
        ...
        this->setCustomPub(this->m_node->Advertise<gazebo::msgs::JointWrench>(topicName),
                           topicName);
        replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
    }
...
std::pair<std::shared_ptr<gazebo_msgs_JointWrenchSubscriber>, bool> gazebo_msgs_JointWrenchCustomMessageHandler::insert(
    std::string const& topic_name,
    std::shared_ptr<gazebo_msgs_JointWrenchSubscriber> subsciberPointer) {
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        auto insertResult =
...
std::shared_ptr<gazebo_msgs_JointWrenchSubscriber> gazebo_msgs_JointWrenchCustomMessageHandler::find(std::string const& topic) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
std::string gazebo_msgs_JointWrenchCustomMessageHandler::initSubscriber(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (iter != m_subscriberPtrMap.end()) {
        ...
        auto insertResult =
            this->insert(topicName, std::make_shared<gazebo_msgs_JointWrenchSubscriber>(topicName));
...
std::string gazebo_msgs_JointWrenchCustomMessageHandler::publishCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
    ...
    if (pubPointer) {
        gazebo::msgs::JointWrench gazebo_msgs_JointWrench_message;
        if (gazebo_msgs_JointWrench_message.ParseFromString(dataString)) {
            pubPointer->Publish(gazebo_msgs_JointWrench_message);
            // assign no error
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError_NONE);
        } else {
...
std::pair<bool,std::string>  gazebo_msgs_JointWrenchCustomMessageHandler::subscribeCustomMsg(mw::internal::robotics::gazebotransport::Packet const& msgContent) {
