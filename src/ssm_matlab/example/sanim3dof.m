function [sys,x0,str,ts,simStateCompliance] = sanim3dof(t,x,u,flag,Config)
...
switch flag,
  ...
  case 9,
     ...
     if Config.Animenable
       ...
       h=findobj(h_f,'type','uimenu','Tag','figMenuCameraToolbar');
...
%
   ...
   set(handle.axes(1), 'SortMethod','childorder',...
       'Color',[.94 .94 .94]);
...
%
   ...
   a = size(vert);
   dum   = (attitude*vert'+[u(1) 0 u(2)]'*ones(1,a(1)))';
...
%
   ...
   for i = 1:num
     xcraft{i}=[s_fore s_fore s_fore+l_fore s_fore+l_fore ]-c_g; %#ok<AGROW>
     ycraft{i}=[yc_range(i) yc_range(i+1) yc_range(i+1) yc_range(i)]; %#ok<AGROW>
     zcraft{i}=[zc_range(i) zc_range(i+1) zc_range(i+1) zc_range(i)]; %#ok<AGROW>
   end
   count=num+1;   
%
% Throttle Shape
%
   ...
   for i = 1:num
     xcraft{count}=[len-thr_len len-thr_len len len]-c_g; %#ok<AGROW>
     ycraft{count}=[yc_range(i) yc_range(i+1) yc_range2(i+1) yc_range2(i)]; %#ok<AGROW>
     zcraft{count}=[zc_range(i) zc_range(i+1) zc_range2(i+1) zc_range2(i)]; %#ok<AGROW>
     count=count+1;
   end
...
%
   for i = 1:num
     xcraft{count}=[s_fore s_fore 0 s_fore]-c_g; %#ok<AGROW>
     ycraft{count}=[yc_range(i) yc_range(i+1) 0 yc_range(i)]; %#ok<AGROW>
     zcraft{count}=[zc_range(i) zc_range(i+1) 0 zc_range(i)]; %#ok<AGROW>
     count=count+1;
   end
%
% Wing shapes 
...
%
   ...
   for i = 1:count
     x = [x xcraft{i}']; %#ok<AGROW>
     y = [y ycraft{i}']; %#ok<AGROW>
     z = [z zcraft{i}']; %#ok<AGROW>
   end
